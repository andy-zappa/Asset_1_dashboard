...
/* 🎯 플로팅 배너 CSS 수정 (상단 우측 바짝 붙이기, gap 0px, 패딩 최소화) */
...
div[data-testid="stHorizontalBlock"]:has(span#zappa-floating-menu) button { margin: 0 !important; padding: 0 3px !important; width: auto !important; background: transparent !important; border: none !important; border-radius: 0 !important; height: 24px !important; min-height: 24px !important; color: #9ca3af !important; font-size: 13.5px !important; font-weight: normal !important; white-space: nowrap !important; box-shadow: none !important; display: flex !important; align-items: center !important; justify-content: center !important; }
...
# =========================================================
    ...
    if data and "_insight" in data:
        ...
        # 🎯 [수정 1] 플로팅 메뉴 버튼 텍스트의 띄어쓰기(간격) 한 칸씩 제거
...
# =========================================================
    ...
    # 🎯 [아이콘 삽입 기능] 1순위: Clearbit API 로고 매핑, 2순위: UI-Avatars 이름 기반 원형 뱃지
    LOGO_MAP = {
        "삼성전자": "samsung.com", "KODEX 레버리지": "samsungfund.com", "현대차": "hyundai.com",
    ...
    def get_logo_html(nm):
        if nm in ["예수금", "[ 합  계 ]"]: return ""
        domain = LOGO_MAP.get(nm)
        safe_nm = urllib.parse.quote(nm)
        # 종목이 리스트에 없을 경우 글자로 예쁜 동그라미 아이콘을 대신 만들어주는 자동 대처 API
        fallback_url = f"https://ui-avatars.com/api/?name={safe_nm}&background=f1f5f9&color=334155&rounded=true&size=40&font-size=0.4"
        
        if domain:
            img_url = f"https://logo.clearbit.com/{domain}?size=40"
            return f"<img src='{img_url}' onerror=\"this.onerror=null; this.src='{fallback_url}';\" style='width:18px; height:18px; border-radius:50%; vertical-align:middle; margin-right:8px; margin-bottom:2px; box-shadow: 0 1px 2px rgba(0,0,0,0.15);'>"
        else:
            return f"<img src='{fallback_url}' style='width:18px; height:18px; border-radius:50%; vertical-align:middle; margin-right:8px; margin-bottom:2px; box-shadow: 0 1px 2px rgba(0,0,0,0.15);'>"
    ...
    # 🎯 [수정 2] 일반계좌 도넛 라벨 국내투자 텍스트 위로 10px 상향 (bottom: 42px), 우측 2px 이동 (left: -20px)
    ...
    # 🎯 [수정 1] 플로팅 메뉴 버튼 텍스트 간격(띄어쓰기) 제거 및 컴팩트 적용
    b1, b2, b3, b4, b5, b6 = st.columns(6)
    with b1:
        st.markdown("<span id='zappa-floating-menu'></span>", unsafe_allow_html=True)
        ...
        with st.expander(f"📂 [ {nm_table_expander[k]} ] 종목별 현황", expanded=False):
            ...
            for i in ([s_data] + items + [cash_item]):
                ...
                if is_s:
                    ...
                    row += f"<td style='text-align:left; padding-left:20px;'><span style='font-size:16px; margin-right:6px; vertical-align:middle;'>💵</span>{display_nm}</td>"
                else:
                    logo_html = get_logo_html(display_nm)
                    row += f"<td style='text-align:left; padding-left:20px;'>{logo_html}{display_nm}</td>"
