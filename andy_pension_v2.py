...
def generate_asset_data():
    ...
    try:
        ...
        us_stats, kr_stats = [], []
        us_total_gain, kr_total_gain = 0, 0
        us_d1, kr_d1 = 0, 0
        ...
        if kr_total_gain >= us_total_gain:
            b3 = f"• 종목별로 보면, 미국 ETF 장기간 횡보 속에({us_str}) 코스피 등 한국 ETF가 전체 평가 손익을 주도하고 있습니다."
        else:
            b3 = f"• 종목별로 보면, 한국 ETF 횡보 속에({kr_str}) 미국 ETF가 전체 평가 손익을 주도하고 있습니다."
        ...
        # [5] 앱 멈춤의 원인인 API를 제거하고 전일비 기반 알고리즘으로 시황 완벽 대체
        if us_d1 > 0 and kr_d1 > 0:
            market_flow = "간밤 미국 기술주와 나스닥 지수가 상승세를 보인 훈풍이 한국 증시로 이어지며, 코스피 관련 ETF 역시 동반 상승하는 강세장을 기록 중입니다. 현재의 긍정적 흐름을 유지하되 많이 오른 종목의 부분 익절을 통한 리밸런싱을 고려해 볼 시점입니다."
        elif us_d1 < 0 and kr_d1 > 0:
            market_flow = "간밤 미국 빅테크 및 나스닥 증시가 조정을 받으며 하락했으나, 투자 자금이 한국 등 신흥국으로 순환매되며 코스피 관련 종목은 오히려 상승 흐름을 주도하고 있습니다. 미국장 변동성에 대비하여 방어력을 점검할 필요가 있습니다."
        elif us_d1 > 0 and kr_d1 < 0:
            market_flow = "미국 기술주와 나스닥은 견조한 상승 흐름을 유지하고 있으나, 외국인 수급 이탈 및 관망세로 인해 코스피가 다소 하락 및 쉬어가는 장세를 보이고 있습니다. 당분간 미국 위주의 포트폴리오 비중을 유지하며 국내 증시의 반등을 대기하는 것이 좋습니다."
        else:
            market_flow = "고용지표 및 매크로 불확실성에 따른 글로벌 증시 변동성이 커진 상황으로, 미국 증시와 한국 코스피가 모두 조정을 겪으며 전반적인 하락세를 보이고 있습니다. 무리한 추격 매수보다는 현금 비중을 확보하며 보수적인 관리가 필요한 시점입니다."
