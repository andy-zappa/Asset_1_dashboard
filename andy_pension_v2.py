...
def generate_asset_data():
    ...
    if token: print("✅ API 토큰 발급 성공! 실시간 ETF 가격을 가져옵니다.")
    else: print("⚠️ API 통신 실패. 기존 세팅된 백업 데이터로 계산을 진행합니다.")

    # =====================================================================
    # 📝 기초 자산 데이터 (수량/매입가 고정, 현재가/전일비는 API로 실시간 덮어쓰기)
    # =====================================================================
    ...
    dc_items = [
        ...
        {"종목명": "삼성화재 퇴직연금이율보증형(3년)", "코드": "-", "수량": "-", "매입가": "-", "현재가": "-", "전일비": 0, "총자산": 90319114, "매입금액": 90000000},
        {"종목명": "현금성자산", "코드": "-", "수량": "-", "매입가": "-", "현재가": "-", "전일비": 0, "총자산": 653113, "매입금액": 653113}
    ]
    ...
    def process_account(items, principal):
        ...
        for it in items:
            ...
            # 주식형 ETF 실시간 API 덮어쓰기
            if token and it.get("코드") != "-":
                cp, dr = get_dom_price(it["코드"], token)
                if cp is not None:
                    it["현재가"] = cp
                    it["전일비"] = dr
    ...
    print("✅ ZAPPA: 절세계좌 실시간 API 동기화 및 저장 성공!")
