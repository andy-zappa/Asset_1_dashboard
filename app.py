...
def clean_label(lbl):
    # 정규식을 이용해 (25.X월) 형태의 날짜 텍스트를 완전히 제거 (요약표 2번용)
...
# [1]표 엑셀 스타일 역 L자 헤더 적용 (-15일전, -30일전 2칸 병합)
h1 = [unit_html, """
...
td15_tot_1 = tot.get('평가손익(-15일전)',0)
td30_tot_1 = tot.get('평가손익(-30일전)',0)

h1.append(f"<tr class='sum-row'><td>[ 합계 ]</td><td>{fmt(ta)}</td><td class='{col(tg)}'>{fmt(tg, True)}</td><td class='{col(td15_tot_1)}'>{fmt(td15_tot_1, True)}</td><td class='{col(td30_tot_1)}'>{fmt(td30_tot_1, True)}</td><td class='{col(ty)}'>{fmt_p(ty)}</td><td>{fmt(to)}</td></tr>")
for k in ['DC', 'IRP', 'PENSION', 'ISA']:
    if k in data:
        a = data[k]
        ad15_acc_1 = a.get('평가손익(-15일전)', 0)
        ad30_acc_1 = a.get('평가손익(-30일전)', 0)
        # 괄호 포함 원본 label 그대로 사용 (폰트 사이즈 축소 없음)
        h1.append(f"<tr><td>{a['label']}</td><td>{fmt(a['총 자산'])}</td><td class='{col(a['총 수익'])}'>{fmt(a['총 수익'],True)}</td><td class='{col(ad15_acc_1)}'>{fmt(ad15_acc_1, True)}</td><td class='{col(ad30_acc_1)}'>{fmt(ad30_acc_1, True)}</td><td class='{col(a['수익률(%)'])}'>{fmt_p(a['수익률(%)'])}</td><td>{fmt(a['원금'])}</td></tr>")
h1.append("</table>")
...
# [2]표 엑셀 스타일 역 L자 헤더 (-1, -7, -15, -30일전 4칸 병합)
h2 = [unit_html, """
<table class='main-table'>
  <tr>
    <th rowspan='2'>계좌 구분</th>
    <th rowspan='2'>총 자산</th>
    <th rowspan='2' class='th-eval'>평가손익</th>
    <th colspan='4' class='th-blank'>&nbsp;</th>
    <th rowspan='2'>수익률</th>
    <th rowspan='2'>매입금액</th>
  </tr>
  <tr>
    <th class='th-week'>-1일전</th>
...
td15_tot = tot.get('평가손익(-15일전)',0)
td30_tot = tot.get('평가손익(-30일전)',0)

h2.append(f"<tr class='sum-row'><td>[ 합계 ]</td><td>{fmt(tot.get('총 자산'))}</td><td class='{col(ag_tot)}'>{fmt(ag_tot, True)}</td><td class='{col(td1_tot)}'>{fmt(td1_tot, True)}</td><td class='{col(td7_tot)}'>{fmt(td7_tot, True)}</td><td class='{col(td15_tot)}'>{fmt(td15_tot, True)}</td><td class='{col(td30_tot)}'>{fmt(td30_tot, True)}</td><td class='{col(ay_tot)}'>{fmt_p(ay_tot)}</td><td>{fmt(tot.get('매입금액합'))}</td></tr>")
for k in ['DC', 'IRP', 'PENSION', 'ISA']:
    if k in data:
        a = data[k]
        ...
        ad15_acc = a.get('평가손익(-15일전)', 0)
        ad30_acc = a.get('평가손익(-30일전)', 0)
        
        # [2]번 표에서는 clean_label을 통해 (25.8월) 등의 날짜 텍스트를 제거
        h2.append(f"<tr><td>{clean_label(a['label'])}</td><td>{fmt(a['총 자산'])}</td><td class='{col(ag_acc)}'>{fmt(ag_acc, True)}</td><td class='{col(ad1_acc)}'>{fmt(ad1_acc, True)}</td><td class='{col(ad7_acc)}'>{fmt(ad7_acc, True)}</td><td class='{col(ad15_acc)}'>{fmt(ad15_acc, True)}</td><td class='{col(ad30_acc)}'>{fmt(ad30_acc, True)}</td><td class='{col(ay_acc)}'>{fmt_p(ay_acc)}</td><td>{fmt(ap_acc)}</td></tr>")
h2.append("</table>")
