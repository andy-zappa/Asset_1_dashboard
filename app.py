...
if "_insight" in data:
    ...
    # =====================================================================
    # 🧠 ZAPPA 동적 시황 분석 알고리즘 (Bullet Icon & No-Colon 패치)
    # =====================================================================
    try:
        acc_rates = []
        for k in FIXED_ACCOUNT_ORDER:
        ...
        zappa_html += f"<div style='margin-bottom: 22px;'><span style='{t_style}'>{bullet} 계좌 현황 분석</span>최근 코스피의 꾸준한 반등 흐름 속에 <strong>{market_lead_txt}</strong> 전체 평가 손익을 주도하고 있습니다. 특히 관련 비중이 높은 <strong>{best_acc_name} 계좌가 전체 수익률(<span class='{col(best_acc_rate)}' style='font-weight:bold;'>{fmt_p(best_acc_rate)}</span>) 1위</strong>를 기록하며 포트폴리오의 실적을 훌륭하게 방어하고 있습니다.</div>"

        zappa_html += f"<div style='margin-bottom: 22px;'><span style='{t_style}'>{bullet} BEST / WORST 종목 점검</span>손익률 최상위인 <strong>{b1_name} (<span class='{col(b1_rate)}' style='font-weight:bold;'>{fmt_p(b1_rate)}</span>)</strong>는 최근 주가 상승 분을 오롯이 누리면서도 배당/인컴 등 특유의 방어적 매력까지 더해져 완벽한 효자 종목으로 활약 중입니다. 반면 <strong>{w1_name} (<span class='{col(w1_rate)}' style='font-weight:bold;'>{fmt_p(w1_rate)}</span>)</strong>를 비롯한 일부 해외 종목은 최근 부각된 미국 관세 우려 및 금리 인하 지연 분위기의 영향으로 상대적으로 부진한 흐름입니다.</div>"
