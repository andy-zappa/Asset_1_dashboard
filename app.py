...
/* =========================================================
   [ZAPPA 플로팅 배너 CSS] - 들여쓰기 꼬임 및 줄바꿈 완전 방지
   ========================================================= */
.zappa-icon { 
...
div[data-testid="stHorizontalBlock"]:has(span#zappa-floating-menu) > div { 
    min-width: 0 !important; 
...
if menu == "1. 복합 대시보드":
    ...
    if data and "_insight" in data:
        ...
        # [수정] 메인 총 자산: 700 굵기 & -0.5px 자간 적용
        html_parts.append(f"        <div style='font-size: 24px; font-weight: 700 !important; color: #111; letter-spacing: -0.5px; line-height: 1;'>{fmt(t_asset)}<span style='font-size: 15px; font-weight: normal; margin-left: 3px; letter-spacing: normal;'>KRW</span></div>")
        ...
        # [수정] 서브 숫자들 원래 굵기(400) 및 18px 핏으로 원상 복구 (왜소함 해결)
        ...
        html_parts.append(f"          <div style='font-size: 18px; font-weight: bold; line-height: 1.1;' class='{col(t_profit)}'>{fmt(t_profit, True)}</div>")
        ...
        # [수정] 통통했던 표 해결 -> font-size 13.5px 원상 복구 적용
