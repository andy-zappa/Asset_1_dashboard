...
with st.sidebar:
    ...
    # 3. 💡 [CSS 전면 수정] 바운스 강화 + 영문 12.5px + 퀀트 로고 디자인
    st.markdown(f"""
    <style>
    /* 상단 상태 박스 */
    .status-box-dark {{
    ...
    div[data-testid="stSidebar"] button[kind="secondary"] {{
        ...
        transition: transform 0.2s cubic-bezier(0.2, 0.8, 0.2, 1), box-shadow 0.2s !important;
    }}
    div[data-testid="stSidebar"] button[kind="secondary"]:hover {{
        background-color: #f0f2f6 !important;
        transform: translateY(-2px) !important; /* 💡 튀어오름 적용 */
        box-shadow: 0 4px 10px rgba(0,0,0,0.1) !important;
    }}
    
    /* 업데이트 버튼 텍스트 해킹 (영문 12.5px) */
    div[data-testid="stSidebar"] button[kind="secondary"] p {{ font-size: 0px !important; }}
    div[data-testid="stSidebar"] button[kind="secondary"] p::before {{
        content: "🔄 업데이트"; font-size: 15.5px !important; font-weight: 800 !important; color: #111111 !important; margin-right: 4px;
    }}
    div[data-testid="stSidebar"] button[kind="secondary"] p::after {{
        content: "(by KIS / UPbit API)"; font-size: 12.5px !important; /* 💡 요청하신 12.5px */
        font-weight: 500 !important; color: #666666 !important;
    }}
    ...
    /* 🤖 퀀트매매 카드 디자인 (image_37be3b.png 완벽 재현) */
    .quant-logo-text {{ font-size: 24px !important; font-weight: 800 !important; color: #111111 !important; line-height: 1 !important; }}
    .quant-subtext {{ font-size: 12px !important; color: #444444 !important; font-weight: 500 !important; margin-top: 2px; }}
    </style>
    """, unsafe_allow_html=True)

    # 4. 업데이트 버튼
    if st.button("UPDATE_DUMMY", key="sidebar_btn_update_v22", use_container_width=True):
        fetch_hybrid_data.clear()
        get_crypto_data.clear()
        st.rerun()

    # 5. 날짜 영역
    st.markdown(f"<div style='text-align: center; font-size: 15px; color: #555555; margin-top: -6px; margin-bottom: 25px;'>{now_str}</div>", unsafe_allow_html=True)

    # 6. 💡 [네비게이션 정정] 인덱스 꼬임 방지를 위한 순서 재정렬
    # 꼬임 원인: "대시보드"가 0번인데 로직에서 1번부터 체크했거나 그 반대일 확률이 높음
    ...
    # 11. 📈 퀀트매매 카드 (image_37be3b.png 디자인 적용)
    st.markdown(f"""
    <div id='card-quant' class='sidebar-card' style='display:flex; flex-direction:row; align-items:center; justify-content:center; gap:15px; padding: 15px 10px; border-radius:12px; background-color:#ffffff; border:1px solid #eeeeee;'>
        <img src='https://cdn-icons-png.flaticon.com/512/4712/4712139.png' style='width:45px; height:45px;'>
        <div style='display:flex; flex-direction:column; align-items:flex-start;'>
            <div class='quant-logo-text'>퀀트매매</div>
            <div class='quant-subtext'>Built & Algo by Andy</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
# =========================================================
