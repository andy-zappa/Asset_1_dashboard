...
with st.sidebar:
    ...
    <div id='card-total' class='sidebar-card sidebar-card-dark'>
        ...
        <div style='text-align: right;'>
            ...
            <div style='font-size:13.5px; margin-top:4px; color:#ff4b4b;'><span class='{col(total_profit)}' style='font-weight:bold;'>{fmt(total_profit, True)}</span> ({fmt_p1(total_rate)})</div>
        </div>
    ...
    <div id='card-pension' class='sidebar-card'>
        ...
        <div style='text-align: right;'>
            <div style='font-size:21px; font-weight:600; color:#111; letter-spacing:-0.5px; line-height: 1.2;'>{fmt(p_asset_all)} <span style='font-size:13px; font-weight:normal; color:#555;'>KRW</span></div>
    ...
    <div id='card-general' class='sidebar-card'>
        ...
        <div style='text-align: right;'>
            <div style='font-size:21px; font-weight:600; color:#111; letter-spacing:-0.5px; line-height: 1.2;'>{fmt(g_asset_all)} <span style='font-size:13px; font-weight:normal; color:#555;'>KRW</span></div>
    ...
    <div id='card-crypto' class='sidebar-card'>
        ...
        <div style='text-align: right;'>
            <div style='font-size:21px; font-weight:600; color:#111; letter-spacing:-0.5px; line-height: 1.2;'>{fmt(c_tot_sum)} <span style='font-size:13px; font-weight:normal; color:#555;'>KRW</span></div>
...
# =========================================================
    ...
    if isinstance(data, dict):
        ...
        t_prof_1ago = 0; t_prof_7ago = 0; t_prof_15ago = 0; t_prof_30ago = 0
        
        for k in FIXED_ORDER:
            if k in data and isinstance(data[k], dict):
                a = data[k]
                t_asset += safe_float(a.get('총 자산', 0))
                t_principal += safe_float(a.get('원금', 0))
                t_buy_total += safe_float(a.get('매입금액', 0))
                
                t_prof_1ago += safe_float(a.get('평가손익(1일전)', 0))
                t_prof_7ago += safe_float(a.get('평가손익(7일전)', 0))
                t_prof_15ago += safe_float(a.get('평가손익(15일전)', 0))
                t_prof_30ago += safe_float(a.get('평가손익(30일전)', 0))
