...
# =========================================================
with st.sidebar:
    # 🎯 현재 무슨 모드로 통신 중인지 알려주는 배지 (안심용)
    if is_oracle_online:
        st.markdown("<div style='text-align:center; padding:6px; background:#e6f4ea; color:#1e8e3e; border-radius:5px; font-size:13px; font-weight:bold; margin-bottom:12px;'>🟢 오라클 서버 실시간 연동 중</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div style='text-align:center; padding:6px; background:#fce8e6; color:#d93025; border-radius:5px; font-size:13px; font-weight:bold; margin-bottom:12px;'>🔴 로컬 백업 데이터 표출 중 (오라클 연결 대기)</div>", unsafe_allow_html=True)

    st.radio("카테고리 선택", ("대시보드", "절세계좌", "일반계좌", "가상자산", "퀀트매매", "모바일관제"), label_visibility="collapsed", key="menu_sel", on_change=on_menu_change)
    ...
    st.markdown(f"""
    <div id='card-quant' class='sidebar-card' style='display:flex; flex-direction:row; align-items:center; justify-content:center; height: 80px; margin-bottom: 12px;'>
    ...
    st.markdown(f"""
    <div id='card-mobile' class='sidebar-card' style='display:flex; flex-direction:row; align-items:center; justify-content:center; height: 60px; margin-bottom: 25px;'>
        <div style='font-size:18px; font-weight:800; color:#1976D2; display:flex; align-items:center; gap:12px; letter-spacing: -0.5px;'>
            📱 모바일 관제
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.current_view == '대시보드':
    ...
    if HAS_PLOTLY:
        ...
        # [일반계좌 통합 데이터 추출 (한국+미국)]
        for k in ['DOM1', 'DOM2', 'USA1', 'USA2']:
            if k in g_data:
                fx = g_data.get('환율', 1443.1) if 'USA' in k else 1
                for item in g_data[k].get('상세', []):
                    nm = item.get('종목명', '').strip()
                    if nm in ['[ 합  계 ]']: continue
        ...
        def render_treemap(data_list, title):
            ...
            # Plotly Treemap의 필수 계층 구조(Root -> Category -> Item) 수동 생성
            labels = ["포트폴리오"]
            parents = [""]
            values = [0]  # Plotly 계산을 위해 부모 노드는 0으로 설정
            colors = ["#1e222d"]
            texts = [""]
            custom_data = [[0,0,0,0,0,0,0]]
            ...
            for cat in df['카테고리'].unique():
                ...
                values.append(0)
                colors.append("#2a2e39")
                texts.append(f"<b style='font-size:14px; color:#e2e8f0;'>{cat}</b>")
                custom_data.append([0,0,0,0,0,0,0])

            # 개별 종목 노드 추가
            for _, r in df.iterrows():
                labels.append(r['종목명'])
                parents.append(r['카테고리'])
                values.append(r['자산'])
                
                # 고급 색상 적용 (수익: 빨강, 손실: 초록, 현금: 짙은 회색)
                ...
                # 볼드(b) 태그 제거 및 span 적용으로 모서리 깨짐 현상 완벽 방지
        ...
        def render_interactive_pie_area(df_pie, title):
            ...
            for i, row in enumerate(df_pie.to_dict('records')):
                ...
                items_js.append({
                    "index": i, "name": row['종목명'], "value": float(row['총자산']), "pct": f"{pct:.1f}%",
                    "logo": logo, "asset": fmt(row['총자산']), "profit": f"{sign}{fmt(row['평가손익'])}",
            ...
            js_chart_data = json.dumps(chart_data, ensure_ascii=False)
            ...
            <head>
                ...
                <style>
                    ...
                    .box {{ background-color: #1a1e28; border-radius: 15px; padding: 25px; position: relative; display: flex; flex-direction: column; height: 500px; box-sizing: border-box; border: 1px solid #2c3140; }}
                    .title {{ color: #fff; font-size: 19px; font-weight: bold; margin-bottom: 20px; }}
                    .content {{ display: flex; flex-direction: row; height: 100%; }}
                    .left-panel {{ flex: 1.1; display: flex; flex-direction: column; padding-right: 20px; border-right: 1px solid #2c3140; }}
                    .hover-panel {{ min-height: 100px; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px dashed #3a3f50; display:flex; flex-direction:column; justify-content:center; transition: opacity 0.2s; }}
                    .list-area {{ flex: 1; overflow-y: auto; padding-right: 10px; }}
                    .list-area::-webkit-scrollbar {{ width: 6px; }}
                    .list-area::-webkit-scrollbar-thumb {{ background-color: #4b5563; border-radius: 3px; }}
                    .chart-area {{ flex: 1.2; height: 100%; position: relative; }}
                    .legend-item {{ transition: all 0.2s ease; }}
                    .legend-item:hover, .legend-item.active {{ background-color: #2d3240; transform: translateX(5px) scale(1.02); box-shadow: 0 4px 10px rgba(0,0,0,0.2); border-left: 3px solid #4CAF50; }}
                    .legend-item.active .leg-name {{ color: #fff !important; font-weight: bold !important; font-size: 16px !important; }}
                    .legend-item.active .leg-pct {{ color: #fff !important; font-size: 16px !important; }}
                </style>
            </head>
            <body>
                <div class="box">
                    ...
                    <div class="content">
                        <div class="left-panel">
                            <div class="hover-panel" id="hover-info">
                                <div style='color:#64748b; font-size:14px; text-align:center; margin-top:20px;'>마우스를 파이 조각이나 목록에 올리면<br>상세 정보가 표시됩니다.</div>
                            </div>
                            <div class="list-area">{list_html}</div>
                        </div>
                        <div class="chart-area" id="pie-chart"></div>
                    </div>
                </div>
                <script>
                    var itemsData = {js_data};
                    var chartData = {js_chart_data};
                    ...
                    function updateHover(idx) {{
                        ...
                        var html = `
                            <div style='display:flex; align-items:center; margin-bottom:8px;'>
                                ${{d.logo}}<span style='color:#fff; font-size:18px; font-weight:bold; letter-spacing:-0.5px;'>${{d.name}}</span>
                            </div>
                            <div style='display:flex; align-items:baseline; margin-bottom:4px; justify-content: flex-end;'>
                                <span style='color:#f1f5f9; font-size:22px; font-weight:bold;'>${{d.asset}}</span><span style='color:#94a3b8; font-size:14px; margin-left:4px;'>원</span>
                                <span style='color:#94a3b8; font-size:15px; margin-left:8px; font-weight:bold;'>(${{d.pct}})</span>
                            </div>
                            <div style='color:${{d.p_class}}; font-size:15px; font-weight:bold; letter-spacing:-0.5px; text-align: right;'>
                                ${{d.profit}} (${{d.rate}})
                            </div>
                        `;
                    ...
                    function clearHover() {{
                        document.getElementById('hover-info').innerHTML = "<div style='color:#64748b; font-size:14px; text-align:center; margin-top:20px;'>마우스를 파이 조각이나 목록에 올리면<br>상세 정보가 표시됩니다.</div>";
                    }}
                    ...
                    var option = {{
                        ...
                        series: [{{
                            ...
                            itemStyle: {{ borderColor: '#1a1e28', borderWidth: 3 }},
                            label: {{ show: true, position: 'inside', formatter: '{{d}}%', color: '#fff', fontSize: 13, fontWeight: 'bold' }},
                            emphasis: {{ 
    ...
    if data and "_insight" in data:
        ...
        for k in FIXED_ACCOUNT_ORDER:
            if k in data:
                ...
                for item in data[k].get('상세', []):
                    ...
                    val = item.get('총 자산', 0)
                    nm = item.get('종목명', '').upper()
                    
                    if '현금' in nm or 'MMF' in nm: 
                        cash_total += val
                    elif any(kw in nm for kw in ['미국', 'S&P', '나스닥', '필라델피아', '다우지수']): 
                        ovs_total += val
        ...
        <div style='font-size: 14.5px; line-height: 1.85; color: #444; padding-left: 0px;'>
            <div style='margin-bottom: 22px;'>
                ...
                <div>
                    현재 전체 포트폴리오 총 손익은 <span class='{col(t_profit)}'><strong>{fmt(t_profit, True)}</strong></span> (<span class='{col(t_rate)}'><strong>{fmt_p(t_rate)}</strong></span>) 이며, 
                    <strong>{best_acc_name}</strong>가 계좌별 수익률 1위를 기록 중입니다. 개별 종목에서는 <strong>{short_name(best_5[0]['종목명']) if best_5 else '주도 종목'}</strong>가 효자 역할을 수행 중이나, 
                    <strong>{short_name(worst_5[0]['종목명']) if worst_5 else '부진 종목'}</strong> 등은 단기 조정을 겪고 있습니다. 
                    총 <strong>{len(tradeable_items)}개</strong> 종목 중 전일비 상승종목은 <strong>{rise_cnt}개</strong>, 하락종목은 <strong>{fall_cnt}개</strong>, 보합 <strong>{flat_cnt}개</strong> 입니다.<br>
                    <span style='font-size:13.5px; color:#555;'>※ 상승종목 : {str_rise}<br>※ 하락종목 : {str_fall}</span>
                </div>
            </div>
            <div style='margin-bottom: 0px;'>
                ...
                <div>
                    간밤 미국 지표의 끈적한 흐름과 연준의 금리 인하 신중론이 겹치며 변동성이 부각되었습니다. 
                    아웃퍼폼 중인 종목에서 일부 차익 실현하여 <strong>현재 {p_cash:.1f}%인 현금 비중을 선제적으로 확대</strong>할 필요가 있습니다.
                </div>
            </div>
        </div>
        """
        ...
        <div style='position: relative; width: 120px; height: 120px; border-radius: 50%; {donut_css} box-shadow: inset 0 0 8px rgba(0,0,0,0.1); border: 1px solid #d0d0d0; flex-shrink: 0; margin: 0 auto;'>
            ...
            <div style='position: absolute; top: 0%; left: 50%; transform: translateX(-50%); font-size: 12.5px; color: #333; text-align: center; line-height: 1.1; font-weight: bold;'>{p_cash:.0f}%<br>현금성자산</div>
            <div style='position: absolute; top: 43px; right: -15px; font-size: 14px; color: #333; text-align: center; line-height: 1.1; font-weight: bold;'>{p_ovs:.0f}%<br>해외투자</div>
            <div style='position: absolute; bottom: 27px; left: -5px; font-size: 14px; color: #fff; font-weight: bold; text-align: center; line-height: 1.1; text-shadow: 0px 0px 3px rgba(0,0,0,0.5);'>{p_dom:.0f}%<br>국내투자</div>
        </div>
        """
        ...
        html_parts.append(f"          <div style='color: #777; font-size: 14px; text-align: right; line-height: 20px;'>평가금액</div>\n          <div style='color: #111; font-size: 18px; font-weight: 400; text-align: right; line-height: 20px;'>{fmt(t_asset - cash_total)}</div>")
        html_parts.append(f"          <div style='color: #777; font-size: 14px; text-align: right; line-height: 20px;'>현금성자산</div>\n          <div style='color: #111; font-size: 18px; font-weight: 400; text-align: right; line-height: 20px;'>{fmt(cash_total)}</div>")
        html_parts.append(f"          <div style='color: #777; font-size: 14px; font-weight: normal; text-align: right; line-height: 20px;'>총 손익</div>\n          <div style='text-align: right;'>\n            <div style='font-size: 18px; font-weight: 600; line-height: 1;' class='{col(t_profit)}'>{fmt(t_profit, True)}</div>")
        ...
        html_parts.append(f"    <div style='margin-top: 20px;'>\n      <div style='display: flex; height: 20px; width: 100%; border-radius: 4px; border: 1px solid #ccc; margin-bottom: 6px; overflow: hidden;'>\n        {render_bar(p_dc, '#b4a7d6')}{render_bar(p_irp, '#f4b183')}{render_bar(p_pension, '#a9d18e')}{render_bar(p_isa, '#ffd966')}\n      </div>")
        ...
        html_parts.append(f"          <span style='font-size: 14px; color: #777; font-weight: normal;'>🎯 은퇴자산 목표 15억 프로젝트</span>\n          <div style='text-align: right;'><span style='font-size: 14px; font-weight: bold; color: #4a90e2;'>{progress_pct:.1f}%</span></div>\n        </div>")
        html_parts.append(f"        <div style='width: 100%; height: 6px; background-color: #e2e2e2; border-radius: 3px; overflow: hidden;'>\n          <div style='width: {progress_pct}%; height: 100%; background: linear-gradient(90deg, #9bc2e6, #4a90e2);'></div>\n        </div>\n      </div>\n    </div>\n  </div>\n</div>")
        ...
        for k in FIXED_ACCOUNT_ORDER:
            if k in data:
                ...
                html_parts.append(f"<div style='font-size: 13.5px; color: #666; font-weight: normal; margin-top: auto; padding-top: 2px; display: flex; justify-content: space-between; align-items: baseline;'><span>* <span style='font-size: 12.5px;'>원금</span> : {fmt(a.get('원금',0))}</span><span><span style='font-size: 16px; font-weight: bold; color: #111;'>{item_count}</span> 종목</span></div>\n</div>\n</a>")
...
# =========================================================
    ...
    c1, c2 = st.columns([8.5, 1.5])
    with c1: 
        st.markdown("<h3 style='margin-top: 5px;'>🚀 이상혁(Andy lee)님 [일반계좌] 통합 대시보드</h3>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='height: 18px;'></div>", unsafe_allow_html=True)
    ...
    <div style='font-size: 14.5px; line-height: 1.85; color: #444; padding-left: 0px;'>
        <div style='margin-bottom: 22px;'>
            ...
            <div>
                현재 전체 포트폴리오 총 손익은 <span class='{col(t_profit)}'><strong>{fmt(t_profit, True)}</strong></span> (<span class='{col(t_rate)}'><strong>{fmt_p(t_rate)}</strong></span>) 이며, 
                <strong>{best_acc_name}</strong> 계좌가 계좌별 수익률 1위를 기록 중입니다. 총 <strong>{len(all_tradeable)}개</strong> 종목 중 0.5% 초과 상승종목은 <strong>{rise_cnt}개</strong>, 하락종목은 <strong>{fall_cnt}개</strong>, 보합종목은 <strong>{flat_cnt}개</strong> 입니다.
            </div>
        </div>
        <div style='margin-bottom: 15px;'>
            ...
            <div>
                최근 국내 시장은 <strong>{short_name(dom_best[0]['종목명']) if dom_best else '국내 우량주'}</strong> 등 일부 우수 종목이 상승을 견인하고 있으나, 
                <strong>{short_name(dom_worst[0]['종목명']) if dom_worst else '일부 조정주'}</strong> 등은 조정을 받고 있습니다. 실적 기반의 리밸런싱을 권고합니다.
            </div>
        </div>
        <div style='margin-bottom: 0px;'>
            <span style='color:#111; font-size:16px; font-weight:bold; display:flex; align-items:center; gap:6px; margin-bottom:6px;'>
            ...
            <div>
                미국 증시는 <strong>{short_name(ovs_best[0]['종목명']) if ovs_best else '빅테크 우량주'}</strong> 위주로 긍정적 흐름을 보이나, 
                <strong>{short_name(ovs_worst[0]['종목명']) if ovs_worst else '단기 하락주'}</strong> 등 부진 섹터는 비중 조절이 필요합니다. 현재 <strong>{(cash_total/t_asset*100) if t_asset>0 else 0:.1f}%</strong>인 현금성(예수금) 비중을 유동적으로 관리하시기 바랍니다.
            </div>
        </div>
    </div>
    """
...
# =========================================================
# 📱 [ Part 5 ] 모바일 관제 화면 (새로 추가된 영역)
# =========================================================
elif st.session_state.current_view == '모바일관제':
    st.markdown("""
        <style>
        .stTable { font-size: 1.0rem !important; }
        th, td { text-align: center !important; }
        .mob-metric { background-color: #f8f9fa; border-radius: 12px; padding: 20px; border: 1px solid #eaeaea; text-align: center; }
        .mob-title { font-size: 1.4rem; font-weight: bold; color: #1976D2; margin: 25px 0 15px 0; border-left: 5px solid #1976D2; padding-left: 10px; line-height: 1; }
        </style>
    """, unsafe_allow_html=True)
        
    c1, c2 = st.columns([8.5, 1.5])
    with c1: 
        st.markdown("<h3 style='margin-top: 5px; margin-bottom: 25px;'>📱 ZAPPA 통합 실시간 관제 (Mobile)</h3>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='height: 18px;'></div>", unsafe_allow_html=True)
        if st.button("🔄 업데이트", use_container_width=True, key="btn_update_mob"):
            with st.spinner("모바일 관제 데이터 갱신 중..."): 
                fetch_hybrid_data.clear()
            st.rerun()

    # 모바일 뷰에서도 data와 g_data를 사용하도록 수정
    if not data and not g_data:
        st.error("📡 데이터 엔진 응답 대기 중... (서버 상태를 확인해주세요)")
    else:
        st.markdown(f"<div style='text-align:right;font-size:13.5px;color:#888;margin-bottom:10px;'>최종 갱신: {to_kst(tot.get('조회시간', ''))}</div>", unsafe_allow_html=True)
        
        c1, c2 = st.columns(2)
        with c1: 
            st.markdown(f"""
                <div class='mob-metric'>
                    <div style='font-size:15px; color:#666; font-weight:bold; margin-bottom:8px;'>전 계좌 총 자산</div>
                    <div style='font-size:26px; font-weight:900; color:#111;'>{fmt(tot.get('총 자산', 0))}원</div>
                </div>
            """, unsafe_allow_html=True)
        with c2: 
            st.markdown(f"""
                <div class='mob-metric'>
                    <div style='font-size:15px; color:#666; font-weight:bold; margin-bottom:8px;'>전 계좌 총 수익</div>
                    <div style='font-size:26px; font-weight:900;' class='{col(tot.get('총 수익', 0))}'>{fmt(tot.get('총 수익', 0), True)}원</div>
                    <div style='font-size:15px; font-weight:bold; margin-top:3px;' class='{col(tot.get('수익률(%)', 0))}'>({fmt_p(tot.get('수익률(%)', 0))})</div>
                </div>
            """, unsafe_allow_html=True)
        ...
        # 📉 모바일 뷰: 일반 자산 현황
        # ---------------------------------------------------------
        st.markdown("<div class='mob-title'>📉 일반 자산 현황</div>", unsafe_allow_html=True)
        nm_table_mob = {'DOM1': '키움증권(국내Ⅰ)', 'DOM2': '삼성증권(국내Ⅱ)', 'USA1': '키움증권(해외Ⅰ)', 'USA2': '키움증권(해외Ⅱ)'}
        
        for k in ['DOM1', 'DOM2', 'USA1', 'USA2']:
            if k in g_data:
                v = g_data[k]
                with st.expander(f"📂 {nm_table_mob.get(k, k)} 상세현황", expanded=True):
                    t_val = v.get('총자산_KRW', 0)
                    p_val = v.get('총수익_KRW', 0)
                    r_val = (p_val / (t_val - p_val) * 100) if (t_val - p_val) > 0 else 0
                    
                    st.info(f"**총자산 {t_val:,.0f} / 총수익 {p_val:+,.0f} ({r_val:+.2f}%)**")
                    
                    if '상세' in v:
                        df_list = []
                        fx_rate = g_data.get('환율', 1400) if 'USA' in k else 1
                        
                        for item in v['상세']:
                            if item.get('종목명') in ['[ 합  계 ]', '예수금']: 
                                continue
                                
                            d = item.copy()
                            if 'USA' in k:
                                d['총 자산'] = d.get('총자산', 0) * fx_rate
                                d['평가손익'] = d.get('평가손익', 0) * fx_rate
                                d['매입가'] = d.get('매입가', 0) * fx_rate
                                d['현재가'] = d.get('현재가', 0) * fx_rate
                            else: 
                                d['총 자산'] = d.get('총자산', d.get('총 자산', 0))
                                
                            df_list.append(d)
                            
                        if df_list:
                            df = pd.DataFrame(df_list)
                            cols = ['종목명', '비중', '총 자산', '평가손익', '수익률(%)', '수량', '매입가', '현재가']
        ...
        # 🛡️ 모바일 뷰: 절세 계좌 현황
        # ---------------------------------------------------------
        st.markdown("<div class='mob-title'>🛡️ 절세 계좌 현황</div>", unsafe_allow_html=True)
        pension_acc_name_mob = {'DC': '퇴직 DC', 'PENSION': '연금저축', 'ISA': 'ISA', 'IRP': 'IRP'}
        
        for k in ['DC', 'PENSION', 'ISA', 'IRP']:
            if k in data:
                v = data[k]
                with st.expander(f"📂 {pension_acc_name_mob.get(k, k)} 상세현황", expanded=True):
                    t_val = v.get('총 자산', 0)
                    p_val = sum(i.get('평가손익', 0) for i in v.get('상세', []) if i.get('종목명') != '[ 합  계 ]')
                    r_val = (p_val / (t_val - p_val) * 100) if (t_val - p_val) > 0 else 0
                    
                    st.info(f"**총자산 {t_val:,.0f} / 총수익 {p_val:+,.0f} ({r_val:+.2f}%)**")
                    
                    if '상세' in v:
                        df_list = [row for row in v['상세'] if row.get('종목명') != '[ 합  계 ]']
                        if df_list:
                            df = pd.DataFrame(df_list)
                            cols = ['종목명', '비중', '총 자산', '평가손익', '수익률(%)', '수량', '매입가', '현재가']
                            df_display = df[[c for c in cols if c in df.columns]]
