...
/* [수정] 박스 둥근 테두리 6px 적용 및 간격 2pt 띄우기 */
div[data-testid='stHorizontalBlock'] { gap: 2pt !important; }
div[data-testid='column'] { padding: 0 !important; }

/* [수정] 상단 업데이트 버튼 및 5개 정렬 버튼 공통 디자인 */
div.stButton>button { 
    font-weight:normal!important; 
    border-radius:6px!important; /* 둥근 테두리 적용 */
...
t1_str = (
    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
    f"**총자산 : {fmt(tot.get('총자산',0))} / "
    f"총수익 : <span class='{col(tot.get('총손익',0))}'>"
...
h1 = [
    unit_html, 
    "<table class='main-table'><tr><th>계좌 구분</th><th>총자산</th>"
    "<th>최초원금</th><th>수익률</th><th>평가금액</th><th>전일비</th></tr>"
]

ty, tg, ta, to = tot.get('수익률(%)',0), tot.get('총손익',0), tot.get('총자산',0), tot.get('원금합',0)
td = tot.get('평가손익(전일비)',0)

ty_str = f"{'▲' if ty>0 else '▼' if ty<0 else ''}{abs(ty):.2f}%"
h1.append(
    f"<tr class='sum-row'><td>[ 합계 ]</td><td>{fmt(ta)}</td><td>{fmt(to)}</td>"
    f"<td class='{col(ty)}'>{ty_str}</td><td class='{col(tg)}'>{fmt(tg, True)}</td>"
    f"<td class='{col(td)}'>{fmt(td, True)}</td></tr>"
)
...
for k in ['DC', 'PENSION', 'ISA', 'IRP']:
    if k in data:
        ...
        row = (
            ...
            f"<td>{fmt(a.get('원금'))}</td><td class='{col(ay)}'>{ay_str}</td>"
            f"<td class='{col(ag)}'>{fmt(ag, True)}</td>"
            f"<td class='{col(ad)}'>{fmt(ad, True)}</td></tr>"
        )
        h1.append(row)
h1.append("</table>")
...
t2_str = (
    "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
    f"**총자산 : {fmt(tot.get('총자산'))} / "
    f"총수익 : <span class='{col(ag_tot)}'>{fmt(ag_tot, True)} "
...
h2 = [
    unit_html, 
    "<table class='main-table'><tr><th>계좌 구분</th><th>총자산</th>"
    "<th>수익률</th><th>평가금액</th><th>매수금액</th></tr>"
]

ay_tot_str = f"{'▲' if ay_tot>0 else '▼' if ay_tot<0 else ''}{abs(ay_tot):.2f}%"
h2.append(
    f"<tr class='sum-row'><td>[ 합계 ]</td><td>{fmt(tot.get('총자산'))}</td>"
    f"<td class='{col(ay_tot)}'>{ay_tot_str}</td>"
    f"<td class='{col(ag_tot)}'>{fmt(ag_tot, True)}</td>"
    f"<td>{fmt(tot.get('매수금액합'))}</td></tr>"
)
...
for k in ['DC', 'PENSION', 'ISA', 'IRP']:
    if k in data:
        ...
        row = (
            ...
            f"<td>{fmt(a.get('총자산'))}</td><td class='{col(ay_acc)}'>{ay_acc_str}</td>"
            f"<td class='{col(ag_acc)}'>{fmt(ag_acc, True)}</td>"
    ...
    if k in data:
        ...
        with st.expander(f"📂 [ {t3_lbl.get(k, a.get('label'))} ] 종목별 현황", expanded=False):
            ...
            t3_str = (
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                f"**총자산 : {fmt(a.get('총자산'))} / "
                f"총수익 : <span class='{col(s_data['평가손익'])}'>"
            ...
            h3.append("<th>비중</th><th>총자산(원)</th><th>평가손익(원)</th><th>수익률</th><th>주식수</th><th>평단가</th><th>금일종가</th></tr>")
