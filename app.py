...
# =========================================================
    ...
    if isinstance(crypto_data, dict) and 'total_asset' in crypto_data:
        c_tot = safe_float(crypto_data.get('total_asset', 0))
        c_krw = safe_float(crypto_data.get('total_krw', 0))
        c_buy = safe_float(crypto_data.get('total_buy', 0))
        c_prof = safe_float(crypto_data.get('total_profit', 0))
        c_rate = safe_float(crypto_data.get('total_rate', 0))
        ...
        if isinstance(coins_list, list):
            for c in coins_list:
                if isinstance(c, dict):
                    c_html += f"<tr><td style='font-weight:bold;'>{c.get('name','')} ({c.get('ticker','')})</td><td>{safe_float(c.get('qty',0)):.8f}</td><td>{fmt(c.get('avg_price',0))}</td><td>{fmt(c.get('curr_price',0))}</td><td style='font-weight:bold;'>{fmt(c.get('eval',0))}</td><td class='{col(c.get('profit',0))}' style='font-weight:bold;'>{fmt(c.get('profit',0), True)}</td><td class='{col(c.get('rate',0))}' style='font-weight:bold;'>{fmt_p(c.get('rate',0))}</td></tr>"
