...
/* 디폴트 상태(체크안됨)일 때 col-code 열을 숨김 */
...
f"<span class='{p_color}' style='font-weight: bold;'>"
f"{format_comma(total.get('총손익', 0), True)} ({total.get('수익률(%)', 0):+.2f}%)</span>"
)
...
f"<span class='{pg_c}' style='font-weight: bold;'>"
f"{format_comma(t_acc_g, True)} ({t_acc_y:+.2f}%)</span>"
)
...
for k in ['DC', 'PENSION', 'ISA', 'IRP']:
if k in data:
acc = data[k]; acc_g = sum(i['평가손익'] for i in acc['상세'] if i['종목명'] != '[ 합계 ]'); acc_p = acc.get('총자산', 0) - acc_g
...
c_s = get_color(acc_val_gain)
        ...
        title3 = (
            ...
            f"<span class='{c_s}' style='font-weight: bold;'>"
            f"{format_comma(acc_val_gain, True)} ({acc_val_yield:+.2f}%)</span>**"
        )
        st.markdown(title3, unsafe_allow_html=True)
        
        html3 = f"""
        <div>
            <input type='checkbox' id='chk_{k}' class='col-toggle-chk'>
            <label for='chk_{k}' class='col-toggle-label'><span class='triangle'>▶</span> 종목코드 (보기/숨기기)</label>
        ...
        for i in acc.get('상세', []):
            ...
            code_val = i.get('코드', '-')
            code_disp = "-" if is_sum or code_val == "-" else code_val
            
            html3 += f"""
                    <tr {row_cls}>
                        <td>{i['종목명']}</td>
                        <td class='col-code' style='color:#0047EB; font-weight:bold;'>{code_disp}</td>
                        <td>{i.get('비중', 0):.1f}%</td>
                        <td>{format_comma(i['평가금액'])}</td>
                        <td class='{c_g}'>{format_comma(i['평가손익'], True)}</td>
                        <td class='{c_y}'>{i['수익률(%)']:+.2f}%</td>
                        <td>{format_comma(i['수량'])}</td>
                        <td>{format_comma(i['평단가'])}</td>
                        <td>{format_comma(i['가격'])}</td>
                    </tr>
            """
        html3 += "</table></div></div>"
