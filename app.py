...
css = """
<style>
/* 🎯 Streamlit 로딩 텍스트만 숨기고 우측 상단 툴바는 유지 */
...
function bindSidebarClicks() {
    ...
    const cardTotal = parentDoc.getElementById('card-total');
    if (cardTotal && !cardTotal.hasAttribute('data-binded')) {
        cardTotal.setAttribute('data-binded', 'true');
        cardTotal.addEventListener('click', function() {
            const target = labels.find(l => l.innerText.includes('대시보드')); if(target) target.click();
        });
    }
    const cardPension = parentDoc.getElementById('card-pension');
    if (cardPension && !cardPension.hasAttribute('data-binded')) {
        cardPension.setAttribute('data-binded', 'true');
        cardPension.addEventListener('click', function() {
            const target = labels.find(l => l.innerText.includes('절세계좌')); if(target) target.click();
        });
    }
    const cardGeneral = parentDoc.getElementById('card-general');
    if (cardGeneral && !cardGeneral.hasAttribute('data-binded')) {
        cardGeneral.setAttribute('data-binded', 'true');
        cardGeneral.addEventListener('click', function() {
            const target = labels.find(l => l.innerText.includes('일반계좌')); if(target) target.click();
        });
    }
}
...
# 🎯 카드 및 요약용 소수점 1자리 렌더링 함수 (0.xx 인 경우만 2자리)
def fmt_p1(v):
    try:
        val = float(v)
        abs_val = abs(val)
...
# =========================================================
with st.sidebar:
    st.radio("카테고리 선택", ("대시보드", "절세계좌", "일반계좌"), label_visibility="collapsed", key="menu_sel", on_change=on_menu_change)
    ...
    # 🎯 30억 달성 프로젝트 진행률 및 카드 텍스트 우측 정렬 반영 (아이콘 🌎)
...
if st.session_state.current_view == '대시보드':
    ...
    if HAS_PLOTLY:
        ...
        donut_colors = ['#ff9999','#66b3ff','#99ff99','#ffcc99','#c2c2f0','#ffb3e6', '#c4e17f', '#76D7C4', '#F7DC6F', '#85C1E9', '#D2B4DE', '#F5B041']
        
        def render_advanced_pie_area(df_pie, title):
            if df_pie.empty: return
            st.markdown(f"<div style='background-color: #1e222d; border-radius: 15px; padding: 15px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);'>", unsafe_allow_html=True)
            st.markdown(f"<div style='color:#fff; font-size:17px; font-weight:bold; margin-bottom:15px; padding-left:5px;'>{title}</div>", unsafe_allow_html=True)
            
            p_col1, p_col2 = st.columns([1.1, 0.9])
            
            with p_col1:
                total_asset = df_pie['총자산'].sum()
                # 🎯 에러 방지 평탄화된 HTML 범례 리스트
                html = "<div style='padding-right: 15px; height: 350px; overflow-y: auto;'>"
                for i, row in enumerate(df_pie.to_dict('records')):
                    ...
                    p_class = "red" if row['평가손익'] > 0 else ("blue" if row['평가손익'] < 0 else "")
                    qty_str = f"보유 {int(row['수량']):,}주" if row['수량'] > 0 else ""
                    html += f"<div style='display:flex; justify-content:space-between; align-items:center; padding:10px 0; border-bottom:1px solid #2a2e39;'><div style='display:flex; align-items:center;'>{logo}<div style='margin-left:5px;'><div style='color:#e2e8f0; font-size:15px; font-weight:bold;'>{row['종목명']}</div><div style='color:#94a3b8; font-size:12px; margin-top:2px;'>{qty_str}</div></div></div><div style='text-align:right;'><div style='color:#f1f5f9; font-size:15px; font-weight:bold;'>{fmt(row['총자산'])}원 <span style='color:#94a3b8; font-size:13px; font-weight:normal;'>({pct:.1f}%)</span></div><div class='{p_class}' style='font-size:13.5px; margin-top:2px; font-weight:500;'>{fmt(row['평가손익'], True)} ({fmt_p(row['수익률'])})</div></div></div>"
                html += "</div>"
                st.markdown(html, unsafe_allow_html=True)
                
            with p_col2:
                pull_list = [0.05] + [0] * (len(df_pie) - 1)  # 1위 종목만 살짝 튀어나오게 (Pull 효과)
                fig = go.Figure(go.Pie(
                    labels=df_pie['종목명'], values=df_pie['총자산'], hole=0.45, pull=pull_list,
                    textinfo='percent', textposition='inside',
                    insidetextfont=dict(size=14, color='white', family='Arial', weight='bold'),
                    hoverinfo='label+percent', showlegend=False, marker=dict(colors=donut_colors[:len(df_pie)])
                ))
                fig.update_layout(margin=dict(t=10, l=10, r=10, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=350)
                st.plotly_chart(fig, use_container_width=True)
                
            st.markdown("</div>", unsafe_allow_html=True)

        render_advanced_pie_area(df_dom_g, "🌱 일반계좌 (한국) 상세 비중")
        render_advanced_pie_area(df_usa_g, "🌱 일반계좌 (미국) 상세 비중")

    else:
        st.error("🚨 **대시보드 차트 렌더링 오류**\n\n'plotly' 및 'pandas' 라이브러리가 설치되어 있지 않습니다.")
...
# =========================================================
    ...
    for k in GEN_ACC_ORDER:
        if k in g_data:
            ...
            h2.append(f"<tr><td>{nm_table[k]}</td><td>{fmt(curr_asset)}</td><td class='{col(ag_acc)}'>{fmt(ag_acc, True)}</td><td class='{col(acc_1d_diff.get(k, 0))}'>{fmt(acc_1d_diff.get(k, 0), True)}</td><td class='{col(diff_7_acc)}'>{fmt(diff_7_acc, True)}</td><td class='{col(diff_30_acc)}'>{fmt(diff_30_acc, True)}</td><td class='{col(ay_acc)}'>{fmt_p(ay_acc)}</td><td>{fmt(a.get('매입금액_KRW', 0))}</td></tr>")
    h2.append("</table>")
    st.markdown("".join(h2), unsafe_allow_html=True)
