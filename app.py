...
/* =========================================================
   [ZAPPA] 대시보드 디자인 (Squeeze 및 여백 최소화 패치)
   ========================================================= */
...
if "_insight" in data:
    ...
    best_5 = all_items[:5]; worst_5 = all_items[::-1][:5]
    ...
    # =====================================================================
    # 🧠 ZAPPA 동적 시황 분석 알고리즘 (좌측 여백 2칸 추가 축소 패치)
    # =====================================================================
    try:
        acc_rates = []
        for k in FIXED_ACCOUNT_ORDER:
        ...
        # [수정] 좌측 여백을 기존 5px에서 -5px 정도로 보정하여 2칸 더 들여쓰기 효과 적용
        ...
        zappa_html += f"<div style='margin-bottom: 22px;'><span style='{t_style}'>{bullet} 계좌 현황 분석</span>최근 코스피의 꾸준한 반등 흐름 속에 <strong>{market_lead_txt}</strong> 전체 평가 손익을 주도하고 있습니다. 특히 관련 비중이 높은 <strong>{best_acc_name} 계좌가 전체 수익률({rate_span(best_acc_rate)}) 1위</strong>를 기록하며 포트폴리오의 실적을 훌륭하게 방어하고 있습니다.</div>"

        zappa_html += f"<div style='margin-bottom: 22px;'><span style='{t_style}'>{bullet} BEST / WORST 종목 점검</span>손익률 최상위인 <strong>{b1_name} ({rate_span(b1_rate)})</strong>는 최근 주가 상승 분을 오롯이 누리면서도 배당/인컴 등 특유의 방어적 매력까지 더해져 완벽한 효자 종목으로 활약 중입니다. 반면 <strong>{w1_name} ({rate_span(w1_rate)})</strong>를 비롯한 일부 해외 종목은 최근 부각된 미국 관세 우려 및 금리 인하 지연 분위기의 영향으로 상대적으로 부진한 흐름입니다.</div>"

        zappa_html += f"<div style='margin-bottom: 0px;'><span style='{t_style}'>{bullet} 거시 경제 및 향후 대응 전략</span>간밤 미국은 예상보다 높은 고용지표로 인해 금리 인하 기대감이 한풀 꺾였고, 트럼프 행정부의 추가 관세 압박까지 겹쳐 빅테크 중심의 하락세가 연출되었습니다. 반면 투자처를 잃은 자금이 한국으로 눈을 돌리며 코스피는 상승 랠리를 이어가고 있습니다. <strong>수익이 급등한 종목은 일부 익절을 통해 현금({p_cash:.0f}%) 비중을 다져두고, 소외된 저평가 자산으로 스마트하게 리밸런싱</strong>하며 변동성에 대비할 것을 권장합니다.</div>"
