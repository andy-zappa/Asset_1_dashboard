...
with st.sidebar:
    ...
    # 1. 날짜(date_part)는 기본 13px를 따릅니다.
    # 2. 요일(day_str)은 12px로 살짝 줄입니다.
    # 3. 시간(time_part)은 12.5px로 정밀하게 세팅합니다.
    now_str_merged = f"[ {date_part}(<span style='font-size: 12.5px;'>{day_str}</span>) / <span style='font-size: 12.0px;'>{time_part}</span> ]"

    st.markdown(f"""
        <div style='text-align: right; padding-right: 2px; margin-top: 15px; margin-bottom: -15px; position: relative; z-index: 10;'>
            <span style='font-size: 12px; color: #888888; font-family: sans-serif; letter-spacing: -0.5px; background-color: transparent;'>
                {now_str_merged}
            </span>
        </div>
    """, unsafe_allow_html=True)
...
# 11. 🤖 AI 퀀트매매 카드 (우측 정렬 및 오류 방지 완전판)
    ...
    <div id='card-quant' class='sidebar-card' style='display:flex; flex-direction:row; align-items:center; justify-content:flex-start; padding-left: 27px; gap:6px; height: 80px; margin-bottom: 5px; background-color:#ffffff; border:1px solid #eeeeee; border-radius:12px;'>
        ...
        <div style='display:flex; flex-direction:column; align-items:flex-end; align-self:flex-end; margin-bottom:3px; line-height:1.0;'>
