...
# [수정] 디자인 설정: 버튼 간격 1.5pt, 둥근 테두리 6px, 호버 시 검은색 강조, 폰트 15px
css = """
<style>
...
# [복원] 좌측 ZAPPA AI 코딩창 및 유채색 로봇 아이콘(🤖✨)
with st.sidebar:
    st.markdown("<div class='sidebar-header'><span style='font-size:32px;'>🤖✨</span><span> ZAPPA AI 코딩 엔진</span></div>", unsafe_allow_html=True)
    try:
        ...
        st.error(f"ZAPPA 엔진 로딩 지연")

# 포맷팅 함수 정의 (구문 오류 수정 완료)
...
# [복원] 절세 자산 현황 요약 (인사이트 박스) - 표 [1] 앞에 배치
if "_insight" in data:
...
for k in ['DC', 'PENSION', 'ISA', 'IRP']:
    if k in data:
        ...
        h2.append(f"<tr><td>{a.get('label').split('(')[0]}</td><td>{fmt(a.get('총 자산'))}</td><td class='{col(ag_acc)}'>{fmt(ag_acc, True)}</td><td class='{col(ay_acc)}'>{fmt_p(ay_acc)}</td><td class='{col(ad_acc)}'>{fmt(ad_acc, True)}</td><td>{fmt(ap_acc)}</td></tr>")
h2.append("</table>")
st.markdown("".join(h2), unsafe_allow_html=True)

# --- [3] 계좌별 상세 내역 ---
st.markdown("<div class='sub-title'>🔍 [3] 계좌별 상세 내역</div>", unsafe_allow_html=True)
# 정밀 정렬 버튼 (1.5pt 간격 동일 적용)
