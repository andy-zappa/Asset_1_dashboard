...
for k in ['DC', 'IRP', 'PENSION', 'ISA']:
    if k in data:
        ...
        h1.append(f"<tr><td>{a['label']}</td><td>{fmt(curr_asset)}</td><td class='{col(a['총 수익'])}'>{fmt(a['총 수익'],True)}</td><td class='{col(ad7_acc_1)}'>{fmt(ad7_acc_1, True)}</td><td class='{col(ad15_acc_1)}'>{fmt(ad15_acc_1, True)}</td><td class='{col(ad30_acc_1)}'>{fmt(ad30_acc_1, True)}</td><td class='{col(a['수익률(%)'])}'>{fmt_p(a['수익률(%)'])}</td><td>{fmt(principal)}</td></tr>")
h1.append("</table>")
...
# [2]표 엑셀 스타일 역 L자 헤더 (명칭에 '比' 추가, 4칸 병합)
h2 = [unit_html, """
<table class='main-table'>
  <tr>
    <th rowspan='2'>계좌 구분</th>
  ...
  <tr>
    <th class='th-week'>-1일전 比</th>
    <th class='th-week'>-7일전 比</th>
    <th class='th-week'>-15일전 比</th>
    <th class='th-week'>-30일전 比</th>
  </tr>
"""]
...
# 증감액 열의 class='{col(...)}' 코드를 제거하여 순수 검정색으로 명시
h2.append(f"<tr class='sum-row'><td>[ 합계 ]</td><td>{fmt(tot.get('총 자산'))}</td><td class='{col(ag_tot)}'>{fmt(ag_tot, True)}</td><td>{fmt(td1_tot, True)}</td><td>{fmt(td7_tot, True)}</td><td>{fmt(td15_tot, True)}</td><td>{fmt(td30_tot, True)}</td><td class='{col(ay_tot)}'>{fmt_p(ay_tot)}</td><td>{fmt(tot.get('매입금액합'))}</td></tr>")
for k in ['DC', 'IRP', 'PENSION', 'ISA']:
    if k in data:
        a = data[k]
        ...
        # clean_label을 통해 (25.8월) 등의 날짜 텍스트를 제거하고, 증감 열은 검정색으로 출력
        h2.append(f"<tr><td>{clean_label(a['label'])}</td><td>{fmt(a['총 자산'])}</td><td class='{col(ag_acc)}'>{fmt(ag_acc, True)}</td><td>{fmt(ad1_acc, True)}</td><td>{fmt(ad7_acc, True)}</td><td>{fmt(ad15_acc, True)}</td><td>{fmt(ad30_acc, True)}</td><td class='{col(ay_acc)}'>{fmt_p(ay_acc)}</td><td>{fmt(ap_acc)}</td></tr>")
h2.append("</table>")
