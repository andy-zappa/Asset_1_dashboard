...
def to_kst(time_str):
    if not time_str or time_str in ['업데이트 필요', '자체 집계 모드']: 
        return str(time_str)
    try:
        # 일반/절세계좌 한글 포맷 텍스트 파싱을 위한 전처리
        t_str = str(time_str).replace('년 ', '-').replace('월 ', '-').replace('일', '')
        t_str = re.sub(r'\(.\)\s*/\s*', '', t_str)
        
        dt = pd.to_datetime(t_str, exact=False)
        # 이미 오라클이 KST로 주므로, UTC 기준 9시간 강제 플러스 로직 제거
...
# =========================================================
# 🚀 [통합] 전역 실시간 업데이트 플로팅 배너 (완벽 일체형 박스)
# =========================================================
upd_time = "업데이트 필요"
...
if st.session_state.current_view in ['절세계좌', '일반계좌', '가상자산']:
    st.markdown("""
    <style>
    /* 1. 겉면 박스를 그려서 일체형처럼 보이게 만듦 (첨부 이미지 디자인 적용) */
    div[data-testid="stVerticalBlock"]:has(span#zappa-global-updater) {
        position: fixed !important;
        top: 75px !important;
        left: 20px !important;
        z-index: 999999 !important;
        background-color: rgba(255, 255, 255, 0.95) !important;
        border: 1.5px solid #888 !important; /* 박스 테두리 */
        border-radius: 12px !important;      /* 둥근 모서리 */
        padding: 8px 12px 10px 12px !important; /* 내부 여백 */
    ...
    /* 2. 안쪽 Streamlit 기본 버튼의 껍데기(테두리, 배경)를 완전히 투명하게 날림 */
    div[data-testid="stVerticalBlock"]:has(span#zappa-global-updater) button {
    ...
    /* 버튼 안의 '🔄 업데이트' 글씨 디자인 */
    div[data-testid="stVerticalBlock"]:has(span#zappa-global-updater) button p {
    ...
    /* 타겟팅용 더미 스팬 숨김 */
    #zappa-global-updater { display: none !important; }
    </style>
    """, unsafe_allow_html=True)

    with st.container():
        # CSS 타겟팅을 위한 숨겨진 마커
        st.markdown("<span id='zappa-global-updater'></span>", unsafe_allow_html=True)
