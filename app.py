...
t1 += f"총수익 : <span class='{pc}' style='font-weight:bold;'>{fmt(tot.get('총손익',0),True)} ({tot.get('수익률(%)',0):+.2f}%)</span>**"
...
t2 += f"총수익 : <span class='{col(ag_tot)}' style='font-weight:bold;'>{fmt(ag_tot,True)} ({ay_tot:+.2f}%)</span>**"
...
for k in ['DC', 'PENSION', 'ISA', 'IRP']:
    if k in data:
        ...
        h2.append(f"<tr><td>{t2_lbl.get(k, a.get('label'))}</td><td>{fmt(a.get('총자산'))}</td><td class='{col(ay_acc)}'>{ay_acc:+.2f}%</td><td class='{col(ag_acc)}'>{fmt(ag_acc,True)}</td><td>{fmt(ap_acc)}</td></tr>")
h2.append("</table>")
