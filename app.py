            ...
            if k in data and isinstance(data[k], dict):
                ...
                if isinstance(details, list):
                    for item in details:
                        if isinstance(item, dict):
                            nm = str(item.get('종목명', ''))
                            if '현금' in nm or '예수금' in nm or 'MMF' in nm:
                                cash_total += safe_float(item.get('총 자산', item.get('총자산', 0)))
