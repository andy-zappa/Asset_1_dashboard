...
# [디자인 설정] 버튼 간격 1.5pt, 둥근 테두리 6px, 호버 시 검은색 강조, 폰트 15px
css = """
<style>
...
.sidebar-header{display:flex;align-items:center;gap:12px;margin-bottom:10px; font-size:20px;font-weight:bold; color: #333;}
...
# [복원] 좌측 ZAPPA AI 코딩창 (유채색 로봇 아이콘 적용)
with st.sidebar:
    if os.path.exists("image_7cea18.png"):
        st.image("image_7cea18.png", width=60)
    else:
        st.markdown("<span style='font-size:40px;'>🤖✨</span>", unsafe_allow_html=True)
    st.markdown("<div class='sidebar-header'>ZAPPA AI 코딩 엔진</div>", unsafe_allow_html=True)
    try:
        ...
        if key:
            ...
            if st.button("개선 사항 반영하기"):
                ...
                st.code(res.text)
    except Exception as e: st.error("엔진 연결 지연")
